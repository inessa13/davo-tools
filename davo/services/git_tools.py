...
def _handler(args, argv):
    ...
    if args.search or args.init:
        ...
        for f in os.listdir(cwd):
            if os.path.isdir(os.path.join(cwd, f)) and os.path.exists(
                os.path.join(cwd, f, ".git")
            ):
    ...
    for project in conf["projects"]:
        ...
        print("[{}{}{}]".format(CLR_WARNING, project["name"], CLR_ENDC))
        result = os.system(
            "git -C {} {}".format(os.path.join(cwd, project["path"]), cmd)
        )
...
def init_parser(parser=None):
    ...
    parser.set_defaults(
        func=lambda namespace: _handler(
            args=namespace,
            argv=namespace.cmd,
        )
    )
