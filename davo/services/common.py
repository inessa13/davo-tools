...
def _make_dirs_sync(
    files, root1, root2, sync_in=False, safe=True, commit=False
):
    processed = 0
...
def init_parser(parser=None, subparsers=None, commands=()):
    ...
    if "keyring" in commands:
        ...
        cmd.set_defaults(
            func=lambda namespace: command_keyring(
                key=namespace.key,
    ...
    if "compare" in commands:
        ...
        cmd.add_argument("-t", "--states", action="store", default="-+~<>r")
        cmd.add_argument(
            "--this", action="store_true", help='alias for -t="+~r<>"'
        )
        cmd.add_argument(
            "--other", action="store_true", help='alias for -t="-~r<>"'
        )
