...
P_SOURCE_CODE = (
    r".*(?P<source_prefix>" + PREFIXES_P + ")(?P<source_num>\d{3,6})\D*.*$"
)
P_SOURCE_CODE2 = (
    r".*(?P<source_prefix>" + PREFIXES_P + ")(?P<source_code>[0-9_]+)\D*.*$"
)
...
def source_datetime(filename, context):
    ...
    if m := re.match(prefixes + r"(\d{8})[_ -](\d{6}).*", name):
        d = datetime.datetime.strptime(
            "{} {}".format(m.group(2), m.group(3)), "%Y%m%d %H%M%S"
        )
        # if d:
    ...
    if m := re.match(prefixes + r"(\d{8})\D+.*", name):
        ...
        if d:
            return m.group(2)
        # return datetime.datetime.strptime(m.group(2), "%Y%m%d")
    return None
...
def _source_classes(_filename, context, match):
    ...
    if class_name.isdigit() and int(class_name) < len(
        context.get("source_match_groups", ())
    ):
...
CLASSES_RE = {
    (
        r"\[exif:(?P<class>[a-zA-Z0-9 _-]+)"
        r"(\:(?P<default>[0-9a-zA-Z-]+))?\]"
    ): lambda f, c, m: (
        _exif_field(c, f, m.group("class"), default=m.group("default") or "")
    ),
...
PATTERNS = {
    "base": {
        ...
        "replace": (
            "IMG_[dto:datetime] "
            "[source:source_prefix][source:source_num].[Ext]"
        ),
        "help": "for most cases",
    },
    "tree": {
    ...
    "model+": {
        "pattern": r".*",
        "replace": (
            "[source:name] [exif:make] [exif:model] [exif:software].[Ext]"
        ),
        "help": "(don't not use for commit) shows model and manufacturer",
    },
    "iphone": {
    ...
    "wa": {
        "pattern": (
            r"(?P<prefix>(IMG|VID))-(?P<source_name>\d+)-"
            r"(?P<source_num>WA\d+)\..*$"
        ),
        "replace": (
            "[source:prefix]_[source:datetime] "
            "[source:source_num].[Ext]"
        ),
