...
from davo import constants, version
...
def init_parser(parser=None, subparsers=None, commands=()):
    ...
    if not commands or "init" in commands:
        ...
        cmd.add_argument("bucket", action="store", help="bucket for sync")
    ...
    common_diff.add_argument(
        "-a", "--all", action="store_true", help="use all modes. ignores -m"
    )
    common_diff.add_argument(
    ...
    common_diff.add_argument(
        ...
        "--modes",
        action="store",
        default=constants.STATES_DIFF_ALL,
    ...
    if not commands or "update" in commands:
        ...
        cmd.add_argument(
            "-q", "--quiet", action="store_true", help="quiet (no interactive)"
        )
        cmd.add_argument(
            "-U", "--upload", action="store_true", help="confirm upload action"
        )
        cmd.add_argument(
            "-D",
            "--download",
        ...
        cmd.add_argument(
            ...
            "--rename-local",
            action="store_true",
            help="confirm rename local file",
        )
        cmd.add_argument(
            "--replace-upload",
            action="store_true",
