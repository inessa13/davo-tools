...
def file_path_info(path, project_root=None, current_root=None):
    ...
    if os.path.isabs(path):
        ...
        key = re.sub("^{}/".format(project_root.replace("\\", "/")), "", path)
...
def _iter_remote_cache(bucket, prefix=None, delimiter=None, depth=None):
    for data in cache.cache.select(
        prefix=prefix, delimiter=delimiter, depth=depth
    ):
...
def _iter_remote(bucket, depth=None, **params):
    for key in bucket.list(**params):
        if not isinstance(key, boto.s3.key.Key) or key.name[-1] == "/":
            continue
